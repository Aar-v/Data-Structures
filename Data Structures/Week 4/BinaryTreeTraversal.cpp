...
class ISolution {
public:
    virtual Collection traverse( H_Tree_node root, Order order, Collection ans={} ) = 0;
};
...
namespace Recursive {
    ...
    public:
        virtual Collection traverse( H_Tree_node root, Order order, Collection ans={} ) override {
            go( root, order, ans );
            return ans;
        }
    private:
        void go( H_Tree_node root, Order order, Collection& ans ){
            if( ! root )
                return;
            if( order == Order::In ){
...
namespace Iterative {
    ...
    private:
        Collection inorder( H_Tree_node node, Collection stack={}, Collection ans={} ){
        ...
        Collection preorder( H_Tree_node node, Collection stack={}, Collection ans={} ){
            if( node )
                stack.push_back( node );
            while( ! stack.empty() ){
                node = stack.back(), stack.pop_back();
        ...
        Collection postorder( H_Tree_node node, Collection stack={}, Collection ans={} ){
            H_Tree_node last{ nullptr };
            while( node || ! stack.empty() ){
                if( node ){
                    stack.push_back( node );
                    node = node->left;
...
int main() {
    ...
    assert( pre1  == pre2  );
    assert( post1 == post2 );
    copy( in1.begin(), in1.end(), ostream_iterator< H_Tree_node >( cout, " " ) ); cout << endl;
    copy( pre1.begin(), pre1.end(), ostream_iterator< H_Tree_node >( cout, " " ) ); cout << endl;
    copy( post1.begin(), post1.end(), ostream_iterator< H_Tree_node >( cout, " " ) ); cout << endl;
    return 0;
}